...
**craft-coach** rides along in your coding sessions (your agent invokes it — you don't). When something from the software-craft canon applies to what's happening *right now* — a "refactor" that quietly changes behavior, generated code sprouting speculative options nobody asked for — it names the concept, cites the source, names the anti-pattern, and leaves the decision to you. It coaches against a contract **you** write: a `COACHING.md` file listing which disciplines you've adopted, which you're trying on, and which are off. No contract, no nagging.
...
Copy the `COACHING-template.md` into your workspace and put *red-green-refactor* under **Trying on** — the coach will surface it gently, once a session, instead of policing you. Run the string-calculator kata (craft-practice) to get the cycle into your hands before using it on real work.
...
The review-format katas are calibration training: the canned kata ships a diff where CI is green and four flaws hide in plain sight (a threshold silently changed inside a "refactor," a test that can't fail, capabilities nothing can send, a swallow-all handler). Run it yourself; then run it with your team and compare what each person caught — it makes review judgment discussable instead of tacit.
...
*Start with:* the canned review kata as a team exercise, and craft-coach where you write the COACHING.md deliberately — a senior's contract is also a statement of standards others will read.
